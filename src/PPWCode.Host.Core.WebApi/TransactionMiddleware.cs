    ...
    : IMiddleware
{
        ...
        [CanBeNull] TransactionalAttribute transactionalAttribute)
    {
        ...
        if (transactionalAttribute is { Transactional: true })
        {
            ...
            Logger.Info(() => $"Created transaction {transaction.GetHashCode()}");
        ...
        [NotNull] ITransaction transaction)
    {
        ...
        {
            Logger.Info(() => $"Transaction {transaction.GetHashCode()} is already closed; no further action performed");
            return;
        }
        ...
        CancellationToken cancellationToken)
    {
        ...
        catch (Exception e)
        {
            ...
            Logger.Error("OnRollbackAsync hook failed with exception: exception is logged but swallowed", e);
        }
        ...
        {
            // log, but swallow exception
            Logger.Error("Actual rollback failed with exception: exception is logged but swallowed", e);
        }
        ...
        {
            // log, but swallow exception
            Logger.Error("OnAfterRollbackAsync hook failed with exception: exception is logged but swallowed", e);
        }
    }
        ...
        CancellationToken cancellationToken)
    {
        ...
        {
            // log exception first
            Logger.Error("HandleCommit failed with exception", e);
        ...
        {
            // log, but swallow exception
            Logger.Error("OnAfterCommitAsync hook failed with exception: exception is logged but swallowed", e);
        }
    }
